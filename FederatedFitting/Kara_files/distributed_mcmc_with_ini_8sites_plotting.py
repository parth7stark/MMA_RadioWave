...
#     band_colors = ['black', 'red', 'blue', 'green', 'magenta']
...
def plot_distributed_data_lc(sites_data, sites_data_UL, theta, include_upper_limits=True):
    ...
    # Use consistent color scheme and markers
    ...
    # Plot model curves for each radio band (same as original functions)
    for i, (band, (fmin, fmax, fcen, marker)) in enumerate(radio_bands.items()):
        ...
        if band_data_exists:
            ...
            # Create custom legend handle for frequency bands (symbol + freq)
    ...
    for site_id, site_data in enumerate(sites_data):
        ...
        for i, (band, (fmin, fmax, fcen, _)) in enumerate(radio_bands.items()):
            ...
            if not band_data.empty:
                ...
                # Overlay scatter points with site-specific marker but band color
                ax.scatter(
                    band_t,
                    band_fnu,
                    marker=site_marker,
    ...
    for site_id in range(len(sites_data)):
        site_marker = site_markers[site_id % len(site_markers)]
        legend_handles.append(Line2D(
            [0], [0],
            marker=site_marker,
    ...
    if include_upper_limits and sites_data_UL:
        for site_id, site_data_UL in enumerate(sites_data_UL):
            ...
            for i, (band, (fmin, fmax, fcen, _)) in enumerate(radio_bands.items()):
                ...
                if not band_data.empty:
                    ...
                    # Plot upper limits with consistent styling
                    ax.scatter(
                        band_t,
                        band_fnu,
                        marker='v',
        ...
        # Add upper limit to legend (consistent with original functions)
        legend_handles.append(Line2D(
            [0], [0],
            marker='v',
            color='black',
    ...
    # Set axis properties (consistent with original functions)
    ax.set(
    ...
    plt.tight_layout()
    plt.savefig(save_folder + '/' + run_name + '_' + 'distributed_lightcurve.png', bbox_inches='tight')
    
    # Remove plt.close() if you want to display the plot
    # plt.close()
...
if __name__ == "__main__":
    ...
    pf_samples_filepath = '../analyze_results/pf_8sites_distributed_dayAll_run5/pf_8sites_distributed_dayAll_run5_distributed_flat_samples.npy'
    ...
    burning = 7000  # or 10_00ls

    # Apply additional burn-in
    flat_samples = flat_samples[burning:]
    # log_prob_filepath = './pf_8sites_distributed_dayAll_run2/pf_8sites_distributed_dayAll_distributed_log_prob.npy'

    log_prob_filepath = '../analyze_results/pf_8sites_distributed_dayAll_run5/pf_8sites_distributed_dayAll_run5_distributed_log_prob.npy'
